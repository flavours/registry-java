...
public class NamespacesController {
    ...
    public static final Namespace STATIC_NAMESPACE = new Namespace("380ca58e-32dc-4a90-831d-b63a57a8f621", "java", List.of());

    @GetMapping(path = "/namespaces/{id}/")
    public ResponseEntity<?> resolveNamespaces(@NotBlank @PathVariable("id") final String id) {
        if (STATIC_NAMESPACE.getId().equalsIgnoreCase(id)) {
            return ResponseEntity.ok(STATIC_NAMESPACE);
        } else {
